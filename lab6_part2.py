...
def philosopher(id: int, chopstick: list): 
    """
       ...
       in this implementation we make sure that the philosphers only pick up both chopsticks if both are readily available at the time
    ...
    for _ in range(6): #to make testing easier, instead of a forever loop we use a finite loop
        ...
        # keep attempting to acquire both chopsticks before proceeding
        while True:
            chopstick[leftChopstick].acquire() # block until left chopsitkc is available
            print(f"DEBUG: philosopher{id} has chopstick{leftChopstick}")
            
            # try to acquire right chopstick immediately aftger acquireing left
            # if right is not available, release left and retry to avoid circular wait
            if chopstick[rightChopstick].acquire(block=False):
                print(f"DEBUG: philosopher{id} has chopstick{rightChopstick}")
                break  # both chopsticks acquired so we can  proceed to eat
            else:
                # right chopstick unavailable so we release left to avoid holding and waiting
                chopstick[leftChopstick].release()
                print(f"DEBUG: philosopher{id} released chopstick{leftChopstick}, retrying")
